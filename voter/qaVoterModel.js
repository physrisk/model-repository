class QAVoterModel {
    ...
    singleStep() {
        let x,y,spinZero,qi,united,conform;
        // pick random agent
        x=Math.floor(Math.random()*this.width);
        y=Math.floor(Math.random()*this.height);
        // check if the agent will conform
        conform=1;
        if(Math.random()<this.pAnticonform) {
            conform=-1;
        }
        ...
        if(united) {
            // if they are united, then adopt their opinion (if decided to conform)
            //         or adopt the opposite opinion (if decided to not conform)
            this.spinArray[y][x]=conform*spinZero;
        }
        this.globalSpin+=this.spinArray[y][x];
    }
    getRandomDirection() {
