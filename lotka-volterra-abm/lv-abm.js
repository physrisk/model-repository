...
function iteracija() {
	time++;
	for(var i=0;i<iterations;i++) vienaIteracija();
	if(time>400) {
		nWArr.splice(0,1);
		nRArr.splice(0,1);
	}
...
function populateField() {
	...
	for(var j=0;j<fieldLY;j++) tfield.push(0);
	for(var i=0;i<fieldLX;i++) field.push(tfield.slice(0,fieldLY));
	for(var i=0;i<Nrabbits;i++) placeAgentInEmpty(-1);
	for(var i=0;i<Nwolves;i++) placeAgentInEmpty(1);
}

function drawField() {
	g.fillStyle="rgb(0,255,0)";
	g.fillRect(0,0,fieldLX*3,fieldLY*3);
	for(var i=0;i<fieldLX;i++) {
		for(var j=0;j<fieldLY;j++) {
			if(field[i][j]==1) {
