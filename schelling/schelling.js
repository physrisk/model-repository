...
function drawField() {
    ...
    for(var i=0;i<fieldL;i+=1) {
        for(var j=0;j<fieldL;j+=1) {
            ...
            if(getSpin(i,j)!=0 && happySpin(i,j)) numberOfHappy+=1;
...
function similaritySpin(x,y) {
    ...
    if(getSpin(x-1,y)==curSpin) similarSpins+=1;
    else if(!(getSpin(x-1,y)==0)) otherSpins+=1;
    if(getSpin(x+1,y)==curSpin) similarSpins+=1;
    else if(!(getSpin(x+1,y)==0)) otherSpins+=1;
    if(getSpin(x,y-1)==curSpin) similarSpins+=1;
    else if(!(getSpin(x,y-1)==0)) otherSpins+=1;
    if(getSpin(x,y+1)==curSpin) similarSpins+=1;
    else if(!(getSpin(x,y+1)==0)) otherSpins+=1;
