                ...
                if(isInfected(i,j)) {
                    if(Math.random()<pih) tfield.push(0);
                        ...
                        if(leftPopulation(i,j)) {
                            dead[0]+=1;
                            if(isVaccinated(i,j)) dead[2]+=1;
                        } else {
                            dead[1]+=1;
                            if(isVaccinated(i,j)) dead[3]+=1;
                        }
                    } else {
                        tfield.push(1);
                        if(leftPopulation(i,j)) infected[0]+=1;
                        else infected[1]+=1;
                    }
                } else {
                    ...
                    if(ninf>0) {
                        var tmp=1.0-phi;
                        var prob=1.0;
                        for(var k=0;k<ninf;k+=1) prob*=tmp;
                        prob=(1-pim)*(1.0-prob);
...
function neighborsInfected(x,y) {
    var n=0;
    if(isInfectedRel(x,y,-1,0)) n+=1;
    if(isInfectedRel(x,y,1,0)) n+=1;
    if(isInfectedRel(x,y,0,-1)) n+=1;
    if(isInfectedRel(x,y,0,1)) n+=1;
    return n;
}
