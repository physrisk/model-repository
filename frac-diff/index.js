...
let mainPlot = new plotlyPlot("mainPlot", ["x", "f(x), f'(x), F(x), D[f(x), d]"], [10, 10, 40, 60]);
let x_step = 2/63;
let x_vals = Array(64).fill(null).map((v, idx) => x_step*idx);

function func(x) {
    return Array(x.length).fill(0).map((v, idx) => x[idx]);
}

function deriv(x) {
    return Array(x.length).fill(1);
}

function prime(x) {
    return Array(x.length).fill(0).map((v, idx) => 0.5*x[idx]*x[idx]);
}
