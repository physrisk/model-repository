...
//functions
//--setting properties
kirmanBassModel.prototype.setEpsilon1=function(min1,max1){
    this.e1min=min1;
    this.e1max=max1;
    this.e1mean=0.5*(min1+max1);
};
kirmanBassModel.prototype.setHerding=function(min,max) {
    this.hmin=min;
    this.hmax=max;
    this.hmean=0.5*(min+max);
};
kirmanBassModel.prototype.setParams=function(min1,max1,hmin,hmax){
    this.setEpsilon1(min1,max1);
    this.setHerding(hmin,hmax);
};
...
kirmanBassModel.prototype.updateStep=function() {
    this.stepDt=this.kappa/(this.population*(2.0*this.e1max+this.hmax));
};
//--runtime functions
