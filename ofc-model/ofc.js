...
function step() {
	var max=-1;
	var newCritCells=[];
	for(var i=0;i<field.length && max<1;i++) {
		for(var j=0;j<field.length && max<1;j++) {
	...
	for(var i=0;i<field.length;i++) {
		for(var j=0;j<field.length;j++) {
			if(field[i][j]>=1) {
				var inArray=critCells.findIndex(function(e,ind,arr){return e[0]==i && e[1]==j;});
	...
	if(newCritCells.length>0) {
		...
		for(var i=0;i<newCrit.length;i++) {
		...
		for(var i=0;i<newCrit.length;i++) {
			if(field[newCrit[i][0]][newCrit[i][1]]>=1) {
				if(appCrit.length==0) appCrit.push([newCrit[i][0],newCrit[i][1]]);
					...
					var trez=newCrit[i];
					var inArray=appCrit.findIndex(function(e,i,a){return e[0]==trez[0] && e[1]==trez[1];});
		...
		if(critCells.length<pdf.length) {
			pdf[critCells.length][1]++;
			quakes++;
		}
		if(timeSeries.length>100) timeSeries.splice(0,3);
		plotFigures();
		for(var i=0;i<critCells.length;i++) {
