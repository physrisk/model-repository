...
function play() {
    for(var i=0;i<100;i+=1) moveBoundaries();
    var rez=totalSpin();
...
function totalSpin() {
    var rez=0;
    for(var i=1;i<bPos.length;i+=1) rez+=(2*(i % 2)-1)*(bPos[i]-bPos[i-1]);
    return spinZero*rez;
}

function moveBoundaries() {
    var bid=[];
    for(var i=0;i<bPos.length-1;i+=1) bid.push(i);
    bid=shuffle(bid);
    for(var i=0;i<bid.length;i+=1) {
        var moveDir=1;
        if(Math.random()<0.5) moveDir=-1;
        if(bid[i]==0 && Math.random()<pFlip && periodic) {
            if(moveDir==1 && bPos[1]>1) {
                for(var j=1;j<bPos.length-1;j+=1) bPos[j]-=1;
            }
        ...
        if(bid[i]>0 && Math.random()<pFlip) {
            if(moveDir==1 && bPos[bid[i]+1]>bPos[bid[i]]+1) bPos[bid[i]]+=1;
            if(moveDir==-1 && bPos[bid[i]-1]<bPos[bid[i]]-1) bPos[bid[i]]-=1;
        }
    }
}
