...
smallWorld.prototype.init=function (graphPlot,console,lang,pCB,nN,pR) {
	...
	this.refreshPlot();
	if(this.language=="lt") Log.write("Kad pradetumėte paspauskite \"Generuoti\"");
	else Log.write("Press \"Rewire\" to start");
...
smallWorld.prototype.attemptRewire=function(){
	...
	if(this.lastNodeId>=this.nNodes) {
		if(this.language=="lt") Log.write('Visi mazgai patikrinti.');
		else Log.write('All nodes were checked.');
	...
	if(Math.random()<this.pRewire) {//add random link for certain node?
		...
		this.refreshPlot();
		if(this.language=="lt") Log.write('Nauja briauna sukurta!');
		else Log.write('New edge created!');
		Log.elem.css({color:"#ff0"});
		return true;
	}
	this.setLastNodeLabels();
	this.refreshPlot();
	if(this.language=="lt") Log.write('Nauja briauna nebuvo sukurta!');
	else Log.write('No new edge created!');
